...
### E. 双轨配对与调度（ticket 07，已冻结）

- **不预合并、不建 paired-cue 模型、不按索引/起点/最近邻/全局 offset 配对**。核心层在 `t = video.currentTime * 1000` 分别计算：
...
- **字号/字形**：中文为主行 `100%`、英文为辅行 `82%`；基础字号按播放器容器高度 `clamp(0.86rem, 6.2cqh, 2.5rem)`（cqh，非 viewport）；中文行 `lang="zh-Hant"`，字体栈 `"Noto Sans TC","PingFang TC","Microsoft JhengHei",sans-serif`，英文系统 sans-serif。
...
- 中英以外的语言对。
- **官方轨的拆句/时间轴再对齐**——DuetSub 走官方双轨 + `0ms` 调度，机翻也逐 cue 保时轴，故不需要沉浸式翻译式的 OT/w6e 拆句与 ET 对齐（两份逆向报告在这上面的篇幅对本项目不适用）。
