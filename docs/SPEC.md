...
- 当一个中文源 cue 明确含多个对话行、当前英文 cue 的可说话单元不足，且该中文源区间还覆盖后续英文 cue 时，才把溢出行依源顺序移到后续有对白的英文 cue；不做翻译语义重排。Max 只有在不少于 **95%** 的中文 cues 都能取得上述唯一候选时才启用英文主轨对齐；低于门槛整条中文轨 fail closed，原生字幕不得提前隐藏。该门槛来自 2026-07-24 两集登录态完整 VTT 的内存核验（258/263，98.10%；英文 CC 310/314，98.73%），不是伪造 fixture。

### F. Overlay 渲染规格（ticket 05，已冻结）

- **行序/间距**：英文上、繁中下，置顶不反转；两行各 `line-height: 1.28`，第二行额外 `0.10em`。
- **字号/字形**：中文为主行 `100%`、英文为辅行 `82%`；基础字号按播放器容器高度 `clamp(13.76px, 6.2cqh, 40px)`（等价于 16px 根字号下的 `0.86rem`–`2.5rem`，但端点不得受宿主页面根字号污染；cqh，非 viewport）；中文行 `lang="zh-Hant"`，字体栈 `"Noto Sans TC","PingFang TC","Microsoft JhengHei",sans-serif`，英文系统 sans-serif。
