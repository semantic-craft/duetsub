...
const ZH_HANT_MESSAGES = {
  ...
  'options.webSearchHelp':
    '使用 Responses API 的 web_search 工具並開啟低強度思考；模型自行決定是否搜尋，可能增加延遲與費用，預設關閉。',
...
const ZH_HANS_MESSAGES: Record<UiMessageKey, string> = {
  ...
  'options.webSearchHelp':
    '使用 Responses API 的 web_search 工具并开启低强度思考；模型自行决定是否搜索，可能增加延迟与费用，默认关闭。',
  'options.baseUrl': 'Base URL',
  'options.apiKey': 'API key',
  'options.model': '模型',
...
const EN_MESSAGES: Record<UiMessageKey, string> = {
  ...
  'options.webSearchHelp':
    'Uses the Responses API web_search tool with low reasoning effort. The model decides whether to search; this may add latency and cost. Off by default.',
  'options.baseUrl': 'Base URL',
  'options.apiKey': 'API key',
  'options.model': 'Model',
  ...
  'toggle.retranslate': 'Retranslate',
