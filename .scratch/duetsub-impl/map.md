...
- **05 HBO Max adapter（已 reopen）**：完整 `playbackInfo + MPD` 映射官方轨，优先英文 CC 并以唯一 active 英文 cue 对齐繁中显示副本；低于 95% 整轨 fail closed。2026-07-24 真人回放发现画面曾不显示及无前缀多行繁中 cue 抢跑；2026-07-25 重载实际 build 已确认画面、双字幕与控制条位置恢复，精确时轴测试已锁定抢跑模式。seek 跳转后没有陈旧 cue，但观察窗口没有新 cue，故完整 stop rule 仍未通过，当前 Status 为 `claimed`；广告 gate 仍按用户明确指示 WAIVED。
