...
- **官方字幕优先。** 播放器选单只列出当前节目经过验证的官方字幕语言；选择语言对本身不会调用机器翻译，AI 重译始终需要单独主动触发。
...
| Disney+ | 支持从当前节目完整的官方 HLS 字幕目录自由配对 |
...
两条官方字幕都可用时，DuetSub 默认不会连接任何翻译服务。只有补位确有需要，或你主动点击 **“用 AI 重译下方字幕”** 时，才会连接已配置的端点。主动重译只发送当前上方字幕，并在已保存的下方语言偏好为英文、简体中文或繁体中文时按该语言生成结果；点击 **“重新加载官方字幕”** 即可恢复官方语言对。

只有标准的英文 / 繁体中文补位需要翻译时，DuetSub 才会把必要的字幕文本发送到你明确配置并授权的端点。千问使用 Responses API，Workspace ID 由用户自行填写；新建千问配置默认使用 `qwen3.7-plus`，已有模型选择保持不变，联网搜索默认关闭。API key、语言偏好和本地翻译缓存均保存在 `chrome.storage.local`。
